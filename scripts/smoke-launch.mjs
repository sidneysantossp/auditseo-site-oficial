...
async function testIndexable(path, expectedCanonical) {
  ...
  const h1Count = count(html, /<h1\b[^>]*>/gi); const canonicalCount = count(html, /<link\b[^>]*rel=["']canonical["'][^>]*>/gi);
  const failures = [];
  if (response.status !== 200) failures.push(`status=${response.status}`); if (!title) failures.push("title ausente no HTML SSR"); if (canonical !== expectedCanonical) failures.push(`canonical=${canonical || "ausente"}`); if (canonicalCount !== 1) failures.push(`canonicals=${canonicalCount}`); if (/noindex/i.test(robots)) failures.push(`robots=${robots}`); if (h1Count !== 1) failures.push(`h1=${h1Count}`);
...
for (const [path, destination] of REDIRECTS) { try { results.push(await testRedirect(path, destination)); } catch (error) { results.push({ path, ok: false, failures: [String(error)] }); } }
for (const path of NOINDEX) { try { results.push(await testNoindex(path)); } catch (error) { results.push({ path, ok: false, failures: [String(error)] }); } }
try { results.push(await testNotFound()); } catch (error) { results.push({ path: "404 probe", ok: false, failures: [String(error)] }); }
...
if (failed.length) { console.error(`Release bloqueado: ${failed.length} check(s) falharam.`); process.exit(1); }
