...
function Collection({ id, eyebrow, title, text, items }: { id: string; eyebrow: string; title: string; text: string; items: Article[] }) {
  return (
...
const libraryMap = [
  ...
  ["tecnica", "Técnica e mensuração"],
  ["protocolos", "Protocolos"],
  ["mercado", "Perguntas do mercado"],
  ["diagnostico-search-ai", "Diagnóstico Search AI"],
  ["cenarios", "Cenários de operação"],
  ["contratacao", "Decisão de contratação"],
] as const;
...
export default function BlogHubPageV2() {
  return (
    <main className="bg-[#11100f] text-[#f8f8f8]">
      ...
      <nav aria-label="Mapa da Biblioteca" className="border-y border-[#b28453]/12 bg-[#151413] px-6 py-5 xl:px-12">
        <div className="mx-auto flex max-w-[1240px] flex-wrap gap-2">
          {libraryMap.map(([id, label]) => (
            <a key={id} href={`#${id}`} className="rounded-full border border-[#b28453]/24 px-4 py-2 text-xs font-semibold text-[#e0d3c3] transition-colors hover:border-[#b28453]/55 hover:text-[#b28453]">
              {label}
            </a>
          ))}
        </div>
