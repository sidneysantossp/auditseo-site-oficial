    ...
    <main className="bg-[#11100f] text-[#f8f8f8]">
      ...
      <section className="bg-[#e0d3c3] px-6 py-16 text-[#11100f] md:py-20 xl:px-12">
        <div className="mx-auto grid max-w-[1240px] gap-6 lg:grid-cols-3">
          {[[<ShieldCheck size={20} />, "Evidência antes da afirmação", "Se existe documentação oficial, ela sustenta a afirmação externa. Se existe incerteza, o texto precisa dizer isso."],[<Search size={20} />, "Pergunta antes da palavra-chave", "O documento nasce de uma pergunta real, de uma decisão ou de um problema que precisa ser explicado com profundidade."],[<BookOpen size={20} />, "Método antes do número", "Benchmarks, métricas e estudos devem publicar amostra, protocolo e limitações antes de usar percentuais como prova de autoridade."]].map(([icon, title, text]) => <article key={String(title)} className="rounded-[22px] border border-[#11100f]/10 bg-[#f4eee5] p-7"><span className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#11100f] text-[#b28453]">{icon}</span><h2 className="mt-5 font-display text-xl font-bold">{title}</h2><p className="mt-4 text-sm leading-[1.7] text-[#11100f]/68">{text}</p></article>)}
        </div>
      </section>
      ...
      <div className="border-y border-[#b28453]/10 bg-[#151413]"><Collection eyebrow="DECISÃO DE CONTRATAÇÃO" title="Conteúdo para empresas que estão escolhendo como operar SEO." text="Critérios para avaliar consultorias, agências, auditorias, times internos e modelos híbridos sem transformar o processo de compra em uma comparação superficial de pacotes e promessas." items={[...buyerArticleList, ...buyerOpsArticleList]} /></div>
